  ...
  <div class="container">
    ...
    <div v-if="step === 1">
      ...
      <div class="step-actions" v-if="!seedsLoading">
        <button class="btn btn-primary" :disabled="selectedIds.size < 3" @click="getRecs">
          获取推荐（已选 {{ selectedIds.size }} 首）
        </button>
      </div>
    </div>
...
async function getRecs() {
  ...
  step.value = 2
  try {
    const data = await api.coldStartRecommend([...selectedIds.value])
