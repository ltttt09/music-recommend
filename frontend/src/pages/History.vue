...
onMounted(async()=>{
  const uid=getUserId()
  try{const[hD,uD]=await Promise.all([api.getUserHistory(uid),api.getUser(uid)]);history.value=hD.items||[];user.value=uD.user;stats.value=uD.stats}catch(e){}finally{loading.value=false}
})
</script>
<style scoped>
