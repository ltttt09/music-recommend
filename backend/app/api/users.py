"""User & recommendation API endpoints (v2 - with feedback, history, cold start)."""
...
def user_history(user_id):
    limit = _int_arg("limit", 50, min_val=1, max_val=200)
...
def liked_tracks(user_id):
    ...
    if current_user_id and current_user_id != user_id:
        return jsonify({"detail": "不能查看其他用户的喜欢记录"}), 403
...
def cold_start_seeds():
    n = _int_arg("n", 20, min_val=5, max_val=50)
...
def update_profile(user_id):
    current_user_id, error = _require_user()
    if error:
        return error
